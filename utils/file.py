...
def text_to_table(path: str) -> tuple[np.ndarray, int, int]:
  ...
  n = len(rows[0].split()) # sets the number of columns
  data = np.zeros((k, n)) # creates a placeholder matrix for "data"

  for i in range(k):
    line = rows[i].split() # creates an array by spliting the line on every whitespace/tab
    for j in range(n):
      data[i][j] = float(line[j]) # assings the corresponding value to the table as a float
