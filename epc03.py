...
# Item 2: Train SVM with C = 1 for problem with 2 input attributes and 51 instances

# Train SVM with C = 1 for item 2
svm_c1_1 = SVC(kernel='linear', C=1)
svm_c1_1.fit(X_1, y_1)

# Plot decision boundary for C = 1 for item 2
plt.figure()
plt.scatter(X_class1_1[:, 0], X_class1_1[:, 1], marker='+', label='Class +1')
plt.scatter(X_class2_1[:, 0], X_class2_1[:, 1], marker='o', label='Class -1')
...
plt.xlabel('Attribute 1')
plt.ylabel('Attribute 2')
plt.title('Decision Boundary for C = 1 (Item 2)')
plt.legend()
plt.savefig("imgs/epc03/item2_decision_boundary_c1.png")

# Train SVM with C = 100 for item 2
svm_c100_1 = SVC(kernel='linear', C=100)
svm_c100_1.fit(X_1, y_1)

# Plot decision boundary for C = 100 for item 2
plt.figure()
plt.scatter(X_class1_1[:, 0], X_class1_1[:, 1], marker='+', label='Class +1')
plt.scatter(X_class2_1[:, 0], X_class2_1[:, 1], marker='o', label='Class -1')
...
plt.xlabel('Attribute 1')
plt.ylabel('Attribute 2')
plt.title('Plot of Data for Item 3')
plt.legend()
plt.savefig("imgs/epc03/item3_plot.png")

# Item 4: Train five SVM with Gaussian kernel for problem with 2 input attributes and 750 instances

# Train SVMs with Gaussian kernel for item 4
...
# Plot accuracies for item 4
plt.figure()
plt.plot(gammas, accuracies, marker='o')
plt.xlabel('Gamma')
plt.ylabel('Accuracy')
plt.title('Accuracy for Different Gamma Values (Item 4)')
